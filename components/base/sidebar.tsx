import { Box, Flex, VStack, BoxProps, useBreakpointValue, Avatar, AvatarGroup, Text, Tooltip, HStack } from "@chakra-ui/react"
...
const UserMenu = () => {
    const [avatarText, setAvatarText] = useState('Login / Signup')
    const [avatarIcon, setAvatarIcon] = useState(<UserIcon color="white" fontSize={{base: '1em', md:'1.5rem'}} />) //TODO add logic for changing the icon to profile picture if exist after auth and to user icon if no profile presents
    const size = useBreakpointValue({base:"sm", md:"md"})
    const onClickHandler = () => {
        console.log('login or profile routing');
    }
    ...
    return (
        <Flex direction={"column"} alignItems={"center"} w={{base:"", md: "100%"}} pos={{base:"relative", md:"absolute"}} bottom={{base:"", md:"5em"}} left={{base:"", md:"0em"}} as="button" onClick={onClickHandler}>
            <AvatarGroup>
                <Avatar bg='#6441F1' size={size} icon={avatarIcon} />
            </AvatarGroup>
